...
``[テーブル].フィールド``のようにエスケープすることができます。

📜[SQLリファレンス > シンタックスルール > sql_name](http://doc.4d.com/4Dv15R5/4D/15-R5/sql-name.300-2977325.ja.html)
...
QUERY BY FORMULA([位置参照情報Ｂ];([都道府県]都道府県名=$value) & \
(([位置参照情報Ｂ]大字町丁目=[大字町丁目]ID) & \
([大字町丁目]市区町村=[市区町村]ID) & \
([市区町村]都道府県=[都道府県]ID)))
```
...
QUERY([位置参照情報Ｂ];[都道府県]都道府県名=$value)
```
